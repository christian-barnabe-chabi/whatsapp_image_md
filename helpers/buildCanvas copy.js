...
function  drawImage1(canvasData) {
  ...
  jimp.read(canvasData.images[0]).then(image => {
    image
    .cover(image1Width, image1Height)
    .quality(100)
    .write(temp, (err, jimp) => {
      console.log(chalk.yellow.bold("Save complete"));
  ...
  // loadImage(canvasData.images[0]).then(image => {
  //   image1Ctx.drawImage(image, 0, 0, image1Width, image1Height);
...
function  drawImage2(canvasData) {
  ...
  loadImage(canvasData.images[1]).then(image => {
...
function  drawImage3(canvasData) {
  ...
  loadImage(canvasData.images[2]).then(image => {
    ...
    .then(() => {
      ...
      const out = fs.createWriteStream(path.join(outFolder, `${canvasData.sku}.jpeg`));
